...
class Rooms(models.Model):
    map_id = models.CharField(u'ID на карте', max_length=20,
                              blank=True, null=True)
    title = models.CharField(u'Название', max_length=30, blank=True,
                             null=True)
    rtype = models.SmallIntegerField(u'Тип аудитории', blank=True,
                                     null=True, default=0)
...
class RoomsParameters(models.Model):
    ...
    int_val = models.IntegerField(u'Значение integer', blank=True,
                                  null=True)
    flo_val = models.FloatField(u'Значение float', blank=True,
                                null=True)
    str_val = models.CharField(u'Значение string', max_length=200,
                               blank=True, null=True)
    dat_val = models.DateTimeField(u'Значение datetime', blank=True,
                                   null=True)
...
class RoomsComments(models.Model):
    room = models.ForeignKey(Rooms, on_delete=models.CASCADE,
                             verbose_name=u'Аудитория')
    comment = models.CharField(u'Комментарий', max_length=50,
                               blank=True, null=True)
    visible = models.BooleanField(u'Виден пользователям',
                                  default=False)
